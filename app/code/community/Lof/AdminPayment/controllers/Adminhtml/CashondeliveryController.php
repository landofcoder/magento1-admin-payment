...
class Lof_AdminPayment_Adminhtml_AdminpaymentController extends Mage_Adminhtml_Controller_Action
{
	protected $zoneType='';
	public function indexAction()
	{
		Mage::register('lof_adminpayment_zone', $this->zoneType);
		if (!$this->zoneType) {
			$this->norouteAction(); return;
		}
		$this->loadLayout()
		->_addContent($this->getLayout()->createBlock('lof_adminpayment/admin_rule'))
		->renderLayout();
	}
	...
	public function editAction()
	{
		...
		$model	= Mage::getModel('lof_adminpayment/'.$this->zoneType)->load($id);
		if ($model->getId() || $id == 0)
		{
			Mage::register('lof_adminpayment_zone', $this->zoneType);
			Mage::register('lof_adminpayment_data', $model);

			$this->loadLayout();
			$this->_addContent($this->getLayout()->createBlock('lof_adminpayment/admin_rule_edit'));
			$this->renderLayout();
		}
		else
		{
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('lof_adminpayment')->__('Item does not exist'));
